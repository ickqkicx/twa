...
public class TodoItemFilter
{
    ...
    public static ValueTask<TodoItemFilter> BindAsync(HttpContext context)
    {
        return ValueTask.FromResult(new TodoItemFilter
        {
            Title = context.Request.Query[nameof(Title)],
            IsComplete = bool.TryParse(context.Request.Query[nameof(IsComplete)], out var r) == true ? r : null
        });
    }
}
